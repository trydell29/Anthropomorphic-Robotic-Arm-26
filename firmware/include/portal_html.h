...
:root{
  ...
  --rule:#C9C6BF; --live:#B86B14; --ok:#3F6B45; --bad:#9E332B;
}
...
.axis{
  ...
  grid-template-columns:2ch 12ch 1fr 6ch 8ch;
...
@media (max-width:560px){
  .axis{grid-template-columns:2ch 1fr 5ch 4ch}
...
<header>
  <div class="hrow">
    <h1>ARA</h1>
    <span class="sub mono">manual control</span>
...
<section>
  ...
  <p class="hint">Re-zero before every measurement session. A skipped step offsets
     every move afterward with no warning anywhere in the system.</p>
</section>

</div>
<script>
const N=13, SERVO_N=11;
let st=null, dragging=-1, sendT={}, lastSent={}, rawTest=false;
...
async function poll(){
  ...
  }catch(e){ el('dLink').className='dot off'; }
}
...
function zeroAxis(i){
  ...
  if(confirm('Declare the current physical position of '+n+' to be zero?')) post('/zero','axis='+i);
}
